...
import { APP_BADGE_CATEGORIES } from "@/lib/constants/app-badges";
...
/** On-chain minted tiers for in-app activity badges (token ids 1000+). */
export async function fetchAppMintedLevelsFromChain(
  client: ChainReader,
  address: string
  ...
  for (const cat of APP_BADGE_CATEGORIES) {
