...
function BaseNetworkBadge({ size = "sm" }: { size?: "sm" | "md" }) {
  ...
  return (
    <div className={`${dim} rounded-full bg-[var(--accent)] flex items-center justify-center shrink-0`}>
...
export default function VoucherGiftCard3D({
  ...
  return (
    ...
    >
      ...
      <div
        className={`relative rounded-2xl overflow-hidden border border-white/15 shadow-[0_24px_60px_rgba(0,0,0,0.55)] ${animated && !flat ? "gift-card-idle-3d" : ""}`}
        style={{
      ...
      >
        ...
        <div className={`relative ${compact ? "p-4" : "p-5 sm:p-6"}`}>
          ...
          <p className={`font-black text-white tracking-tight leading-none mb-5 ${compact ? "text-3xl" : "text-4xl sm:text-[2.75rem]"}`}>
            {amountMain}
