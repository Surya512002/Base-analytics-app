...
test.describe("mobile viewport flows", () => {
  ...
  test("explore token page shows swap UI on mobile", async ({ page }) => {
    ...
    const tokenLink = page.locator('a[href*="/explore/token/"]').first();
    if (await tokenLink.count()) {
      await tokenLink.click();
      await expect(page.getByText(/swap|trade|you pay/i).first()).toBeVisible({
        timeout: 30_000,
      });
    } else {
      await page.goto(`${BASE}/explore/token/${WETH}`);
      await expect(page.getByText(/swap|trade|you pay|explore/i).first()).toBeVisible({
        timeout: 30_000,
      });
    }
  });
