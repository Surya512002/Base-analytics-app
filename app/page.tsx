...
// 2. BASESCAN KEY (For Internal Txs)
...
const BOOSTER_ABI = ["function boost() external payable", "function getUserData(address _user) external view returns (uint256, uint256, uint256)"];
...
export default function Page() {
  ...
  const fetchInternalTxs = async (address: string): Promise<BaseScanTx[]> => {
    try {
        ...
        if (data.status === "1" && Array.isArray(data.result)) {
            console.log(`✅ BaseScan: Found ${data.result.length} internal transactions.`);
            return data.result;
        }
    } catch (e) {
  ...
  const analyzeWallet = async (address: string) => {
    ...
    try {
      ...
      setLoadingMsg("Checking Internal Contracts...");
      ...
      const addTxToDay = (dateStr: string) => {
          const current = txsPerDay.get(dateStr) || 0;
          txsPerDay.set(dateStr, current + 1);
      };
      ...
      setWallet({
        ...
        tokensSwapped: uniqueTokens.size, swapCount, 
        contractInteractions, 
        internalTxCount: internalTxs.length,
        score: Math.min(100, finalScore),
        dailyStats, historyDays, weekLabels
      });
  ...
  const handleBoost = async () => {
    ...
    try {
      ...
      const dataWithTracking = `${populatedTx.data}${getBuilderSuffix()}`;
  ...
  const handleGmGn = async (type: 'gm' | 'gn') => {
    ...
    try {
      ...
      let populatedTx;
      if (type === 'gm') populatedTx = await contract.gm.populateTransaction();
      else populatedTx = await contract.gn.populateTransaction();
      const dataWithTracking = `${populatedTx.data}${getBuilderSuffix()}`;
      const tx = await signer.sendTransaction({
  ...
  if (!wallet) return (
    <div className="min-h-screen bg-[#020410] flex flex-col items-center justify-center p-6 text-center text-white relative overflow-hidden">
      ...
      <button onClick={() => setShowConnectModal(true)} disabled={loading} className="w-full max-w-xs bg-[#0052FF] text-white py-4 rounded-full font-black text-lg flex items-center justify-center gap-3 hover:bg-blue-600 transition active:scale-95 shadow-[0_0_40px_-10px_rgba(0,82,255,0.5)] z-10">
  ...
  return (
    <main className="min-h-screen bg-[#020410] p-4 lg:p-8 font-sans text-slate-200 pb-32">
      ...
      <div className="bg-linear-to-r from-blue-950/40 to-[#020410] rounded-3xl p-1 shadow-2xl mb-8 border border-blue-900/30 relative overflow-hidden group">
            ...
            <div className="bg-[#0A1024]/80 backdrop-blur-xl rounded-[20px] p-6 sm:p-8 relative z-10">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-8">
                    <div>
                        ...
                        <div className="flex gap-4 items-center">
                            <div className="bg-[#020410]/50 px-4 py-2 rounded-lg border border-blue-900/50">
                            ...
                            <div className="bg-[#020410]/50 px-4 py-2 rounded-lg border border-blue-900/50 text-right">
                    ...
                    <div className="flex flex-col gap-2 w-full md:w-auto">
                        <button onClick={handleBoost} disabled={txLoading} className="w-full px-8 py-4 rounded-xl font-bold text-sm bg-[#0052FF] text-white hover:bg-blue-600 shadow-[0_0_20px_-5px_rgba(0,82,255,0.3)] transition active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                ...
                <div>
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <div className="flex gap-2">
                            ...
                            <button onClick={shareNative} className="bg-blue-950/30 text-blue-300 px-3 py-1 rounded-full text-[10px] font-bold flex items-center gap-1.5 hover:bg-blue-900/50 border border-blue-900/30 transition-all"><Share2 size={10}/> Share</button>
                        </div>
                    </div>
                ...
                <div className="grid grid-rows-7 grid-flow-col gap-1.5 h-36 relative z-10 min-w-max">
                    {wallet.dailyStats.map((stat, i) => (
                        <div 
                            ...
                            title={`${stat.date}: ${stat.count} Txs`} 
                            className={`w-3 h-3 rounded-xs cursor-pointer hover:scale-125 transition-all duration-150 
                                ${stat.count === 0 ? 'bg-blue-950/30 hover:bg-blue-900' : ''}
                                ${stat.intensity === 1 ? 'bg-[#0052FF]/40' : ''}
                                ${stat.intensity === 2 ? 'bg-[#0052FF]/60' : ''}
                                ${stat.intensity === 3 ? 'bg-[#0052FF]/80' : ''}
                                ${stat.intensity === 4 ? 'bg-[#0052FF] shadow-[0_0_8px_-1px_rgba(0,82,255,0.8)]' : ''}
                            `}
                        ></div>
                    ))}
                </div>
      ...
      <div className="bg-[#0A1024] rounded-3xl p-6 shadow-sm border border-blue-900/30">
...
function StatCard({ label, value, icon, full, highlight }: { label: string, value: string, icon: React.ReactNode, full?: boolean, highlight?: boolean }) {
    return (
        <div className={`bg-[#0A1024] p-5 rounded-2xl border border-blue-900/20 flex flex-col justify-between group hover:border-[#0052FF]/50 transition-all ${full ? 'col-span-1' : ''}`}>
