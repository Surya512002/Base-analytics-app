...
import { 
  ...
  Shield, Crosshair, Hexagon, Crown, Diamond
} from 'lucide-react';
...
// --- ACHIEVEMENT BLUEPRINT WITH CUSTOM NAMES ---
const ACHIEVEMENTS = [
  { id: 'score', name: 'Onchain Rank', icon: '👑', unit: 'Score', thresholds: [10, 30, 60, 75, 85], tierNames: ["Base Shrimp", "Base Dolphin", "Base Shark", "Base Whale", "Base God"] },
  { id: 'age', name: 'Pioneer', icon: '🕰️', unit: 'Days Old', thresholds: [10, 30, 90, 180, 365], tierNames: ["Newcomer", "Explorer", "Settler", "Veteran", "Early Adopter"] },
  { id: 'name', name: 'Identity', icon: '🏷️', unit: 'Basename', thresholds: [1], tierNames: ["Verified"] },
  { id: 'days', name: 'Diamond Hands', icon: '💎', unit: 'Days Active', thresholds: [10, 50, 100, 200, 365], tierNames: ["Tourist", "Resident", "Citizen", "Veteran", "Immortal"] },
  { id: 'contract', name: 'Base Builder', icon: '🏗️', unit: 'Txs', thresholds: [10, 50, 100, 500, 1000], tierNames: ["Tinkerer", "Apprentice", "Engineer", "Architect", "Master Builder"] },
  { id: 'volume', name: 'Whale Alert', icon: '🐋', unit: 'ETH', thresholds: [0.001, 0.01, 0.1, 1.0, 5.0], tierNames: ["Guppy", "Shrimp", "Dolphin", "Shark", "Leviathan"] },
  { id: 'txs', name: 'Power User', icon: '⚡', unit: 'Total Txs', thresholds: [10, 50, 100, 500, 1000], tierNames: ["Spark", "Bolt", "Surge", "Lightning", "Storm"] },
  { id: 'swaps', name: 'DeFi Degen', icon: '🔄', unit: 'Swaps', thresholds: [3, 10, 25, 50, 100], tierNames: ["Swapper", "Trader", "Provider", "Yield Farmer", "DeFi God"] },
  { id: 'nfts', name: 'Collector', icon: '🖼️', unit: 'NFTs', thresholds: [3, 10, 25, 50, 100], tierNames: ["Scout", "Gatherer", "Curator", "Connoisseur", "NFT Whale"] },
  { id: 'streak', name: 'Streak Master', icon: '🔥', unit: 'Days', thresholds: [3, 7, 14, 30, 100], tierNames: ["Match", "Flame", "Blaze", "Inferno", "Supernova"] },
  { id: 'boosts', name: 'XP Booster', icon: '🔋', unit: 'Boosts', thresholds: [5, 10, 25, 50, 100], tierNames: ["Novice", "Supporter", "Fanatic", "Champion", "Apex"] },
];

// --- UNIQUE GAMING LOGOS & VIBRANT GRADIENTS ---
const getLevelIcon = (level: number) => {
    switch(level) {
        case 1: return <Shield size={16} strokeWidth={2.5} />;
        case 2: return <Crosshair size={16} strokeWidth={2.5} />;
        case 3: return <Hexagon size={16} strokeWidth={2.5} />;
        case 4: return <Crown size={16} strokeWidth={2.5} />;
        case 5: return <Diamond size={16} strokeWidth={2.5} className="drop-shadow-[0_0_8px_rgba(255,255,255,0.8)]" />;
        default: return <Lock size={14} strokeWidth={2.5} />;
    }
}

const getLevelColors = (level: number, isMinted: boolean, isEarned: boolean) => {
    if (!isEarned) return 'bg-slate-200 border-slate-300 text-slate-400 opacity-50 border-solid';
    
    let baseStyle = '';
    switch(level) {
        case 1: baseStyle = 'bg-gradient-to-br from-slate-400 to-slate-500 border-slate-400 text-white shadow-slate-400/50'; break; // Iron
        case 2: baseStyle = 'bg-gradient-to-br from-amber-600 to-orange-700 border-amber-600 text-white shadow-orange-600/50'; break; // Bronze
        case 3: baseStyle = 'bg-gradient-to-br from-slate-200 to-slate-400 border-slate-300 text-slate-800 shadow-slate-400/50'; break; // Silver
        case 4: baseStyle = 'bg-gradient-to-br from-yellow-300 to-yellow-500 border-yellow-400 text-yellow-900 shadow-yellow-400/50'; break; // Gold
        case 5: baseStyle = 'bg-gradient-to-br from-[#0052FF] to-[#8A2BE2] border-[#0052FF] text-white shadow-[#0052FF]/80'; break; // Legendary
    ...
    if (isMinted) {
        ...
        return `${baseStyle} opacity-90 scale-95 shadow-md border-dashed animate-pulse`; // Pulsing dashed border to attract minting!
    }
}
...
interface WalletData {
  ...
  topTokens: string[]; recommendation: string; badges: Badge[]; recentTxs: AlchemyTransfer[];
  daysOnBase: number;
}
...
export default function Page() {
  ...
  const [mockMinted, setMockMinted] = useState<Record<string, number>>({});
  ...
  const analyzeWallet = async (address: string) => {
    ...
    try {
      ...
      for (const tx of allTransfers) {
        ...
        if (tx.value && (tx.asset === 'ETH' || tx.asset === 'WETH')) {
            ethVolume += tx.value;
        }
      ...
      for (let col = 0; col < totalColumns; col++) {
          ...
          if (MONTHS_3_LETTERS[monthIndex] !== lastMonthLabel) { 
              weekLabels.push(MONTHS_3_LETTERS[monthIndex]); 
              lastMonthLabel = MONTHS_3_LETTERS[monthIndex]; 
          } else {
              weekLabels.push(""); 
          }
      }
  ...
  const handleNativeTx = async (type: 'boost' | 'gm' | 'gn') => {
    ...
    try {
      ...
      if (txValue > BigInt(0)) {
          txParams.value = `0x${txValue.toString(16)}` as `0x${string}`;
      }
  ...
  // --- NEW: BUILDER CODE ATTACHED TO SIMULATED MINT ---
  const mockMintAchievement = (catId: string, targetLevel: number, catName: string, levelName: string) => {
    // We grab your unique builder tracking hex directly
    const builderTrackingHex = getBuilderSuffix();
    
    console.log(`[CONTRACT MINT PAYLOAD SIMULATION]`);
    console.log(`Category: ${catId} | Target Level: ${targetLevel}`);
    console.log(`Payload Suffix Attached: ${builderTrackingHex}`);
    
    showToast(`⏳ Minting ${levelName} ${catName}...`, "");
    
    setTimeout(() => {
        setMockMinted(prev => ({ ...prev, [catId]: targetLevel }));
        showToast(`✅ Successfully minted ${levelName}! (Builder code registered)`, "");
    }, 1500);
  };

  const shareAchievement = (catName: string, levelName: string) => {
    const shareText = `I just minted the ${levelName} ${catName} badge on Base Analytics! 🏆🔵\n\nBuilt by @suryaprakash.farcaster.eth 🎩\n\nMint yours 👇`;
  ...
  const shareNative = async () => {
    ...
    if (navigator.share) { 
        try { 
            await navigator.share({ title: 'My Base Analytics', text: shareText, url: MINIAPP_URL }); 
        } catch {} 
    } else { 
    ...
    <main className="min-h-screen bg-slate-300 p-4 lg:p-8 font-sans text-slate-800 pb-12 relative isolate">
      ...
      <div className="flex bg-slate-200 p-1.5 rounded-2xl mb-8 w-full max-w-sm mx-auto border border-slate-300 shadow-sm">
        <button 
            ...
            className={`flex-1 py-2.5 rounded-xl font-bold text-sm transition-all flex justify-center items-center gap-2 ${activeTab === 'dashboard' ? 'bg-[#0052FF] text-white shadow-md' : 'text-slate-500 hover:text-[#0052FF]'}`}
        >
            <BarChart3 size={16}/> Dashboard
        </button>
        <button 
            onClick={() => setActiveTab('achievements')} 
            className={`flex-1 py-2.5 rounded-xl font-bold text-sm transition-all flex justify-center items-center gap-2 ${activeTab === 'achievements' ? 'bg-[#0052FF] text-white shadow-md' : 'text-slate-500 hover:text-[#0052FF]'}`}
        >
        ...
        <div className="animate-in fade-in slide-in-from-bottom-2">
            ...
            <div className="bg-slate-200 rounded-[20px] p-6 sm:p-8 border border-slate-300 mb-8 shadow-md shadow-slate-400/50">
                    <div className="flex flex-col md:flex-row justify-between items-start mb-8 w-full">
                        ...
                        {selectedDay ? (
                            <div className="bg-slate-300 px-4 py-3 rounded-lg border border-slate-400 shadow-inner">
                                <p className="text-xs text-slate-600 font-bold uppercase">{selectedDay.date}</p>
                                <p className="text-xl font-black text-[#0052FF]">{selectedDay.count} Txs</p>
                            </div>
                        ) : (
                            <div className="flex items-center opacity-70 gap-2 justify-end">
                                <MousePointerClick size={16} className="text-[#0052FF]"/>
                                <p className="text-[10px] text-slate-600 uppercase font-bold">Click a dot for details</p>
                            </div>
                        )}
                        </div>
                    </div>
                    ...
                    <div ref={scrollRef} className="w-full overflow-x-auto pb-4 custom-scrollbar">
                        <div className="grid grid-flow-col gap-1.5 mb-2 relative min-w-max auto-cols-[12px]">
                            {wallet.weekLabels.map((m, i) => (
                                <div key={i} className="text-[9px] font-bold text-slate-500 uppercase text-left w-3 whitespace-nowrap overflow-visible">{m}</div>
                            ))}
                        </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-8">
                ...
                <div className="bg-slate-200 p-5 rounded-2xl border border-slate-300 flex flex-col justify-between col-span-2 lg:col-span-2 relative overflow-hidden shadow-md shadow-slate-400/50">
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <p className="text-[10px] text-slate-600 uppercase tracking-widest font-bold">Special Badges</p>
                    </div>
                    <div className="flex flex-wrap gap-2">
                        {wallet.badges.length > 0 ? wallet.badges.map((badge, i) => (
                            ...
                            <span className="text-sm text-slate-500">No special badges unlocked.</span>
                        )}
                    </div>
                </div>
        ...
        <div className="animate-in fade-in slide-in-from-bottom-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                {ACHIEVEMENTS.map((cat) => {
                    ...
                    const currentMintedLevel = mockMinted[cat.id] || 0;
                        ...
                        <div key={cat.id} className="bg-slate-200 p-6 rounded-[20px] border border-slate-300 shadow-md shadow-slate-400/50 flex flex-col relative overflow-hidden group">
                            ...
                            <div className={`flex ${cat.thresholds.length === 1 ? 'justify-around' : 'justify-between'} items-center mb-6 px-1`}>
                                {cat.thresholds.map((threshold, idx) => {
                                    ...
                                    // Give 1-tier achievements the awesome "Level 5" Legendary color styling
                                    const styleTier = cat.thresholds.length === 1 ? 5 : tierIndex; 
                                    const style = getLevelColors(styleTier, isMinted, isEarned);
                                    const levelIcon = getLevelIcon(styleTier);
                                    
                                    return (
                                        <div key={tierIndex} className="flex flex-col items-center gap-2 relative">
                                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-500 ${style}`}>
                                                {isEarned ? levelIcon : <Lock size={14} />}
                                                {isMinted && (
                                                    <div className="absolute -top-2 -right-2 bg-green-500 text-white text-[10px] font-black w-5 h-5 flex items-center justify-center rounded-full border-2 border-slate-200 z-10 shadow-sm">✓</div>
                                                )}
                                            </div>
                                            <span className={`text-[8px] sm:text-[9px] uppercase font-bold text-center w-full ${isMinted ? 'text-[#0052FF]' : 'text-slate-500'}`}>
                                                {cat.tierNames[idx]}
                                            </span>
                                        </div>
                                    );
                            ...
                            <div className="flex gap-2 mt-auto">
                                <button 
                                    onClick={() => mockMintAchievement(cat.id, nextMintTarget, cat.name, cat.tierNames[nextMintTarget - 1])}
                                    disabled={!canMintNext}
                                ...
                                >
                                    {currentMintedLevel === cat.thresholds.length ? 'Fully Minted 👑' : canMintNext ? `Mint ${cat.tierNames[nextMintTarget - 1]}` : `${cat.tierNames[nextMintTarget - 1]} Locked`}
                                </button>
                                {currentMintedLevel > 0 && (
                                    <button 
                                        onClick={() => shareAchievement(cat.name, cat.tierNames[currentMintedLevel - 1])}
                                        className="bg-slate-300 text-[#0052FF] p-3 rounded-xl hover:bg-slate-400/50 transition border border-slate-400 shadow-sm"
                                    >
                                        <Share2 size={18}/>
                                    </button>
                                )}
