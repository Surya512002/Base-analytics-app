...
Check your stats on Base Analytics by @${CREATOR_USERNAME}`;
  ...
  const connectWallet = async () => {
    ...
    if (ethWindow.ethereum) {
      try {
        ...
        // Auto-fetch profile if not logged in
        if (!user && !engagement) {
            setLoading(true);
  ...
  if (!displayUser && !engagement) return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center p-6 text-center">
      <div className="w-16 h-16 bg-blue-600 rounded-2xl mb-6 flex items-center justify-center shadow-xl"><TrendingUp className="text-white" size={32} /></div>
      <h1 className="text-2xl font-black text-slate-900 mb-2 italic tracking-tighter">BASE ANALYTICS</h1>
      <div className="space-y-4 w-full max-w-xs">
          <NeynarAuthButton />
          <button onClick={connectWallet} className="w-full bg-slate-900 text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-slate-800 transition shadow-lg"><Wallet size={18} /> Connect Wallet</button>
      </div>
    </div>
  );
